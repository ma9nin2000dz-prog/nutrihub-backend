...
const MacroGoalProgressCircle = ({ protein, carbs, fat, goal, totalConsumed, size = 120 }) => {
  const strokeWidth = 12;
  const radius = (size - strokeWidth) / 2;
  const circumference = 2 * Math.PI * radius;

  const goalVal = goal || 1200;
  const currentTotal = totalConsumed || 0; 

  const totalProgressFactor = Math.min(currentTotal / goalVal, 1);

  const pCal = protein * 4;
  const cCal = carbs * 4;
  const fCal = fat * 9;
  const sumMacros = pCal + cCal + fCal || 1;

  const pLength = (pCal / sumMacros) * (circumference * totalProgressFactor);
  const cLength = (cCal / sumMacros) * (circumference * totalProgressFactor);
  const fLength = (fCal / sumMacros) * (circumference * totalProgressFactor);
    ...
    <View style={{ alignItems: 'center', justifyContent: 'center' }}>
      <Svg width={size} height={size}>
        ...
        <Circle
          ...
          fill="none"
          strokeLinecap={cLength > 5 ? "round" : "butt"}
        ...
        <Circle
          ...
          fill="none"
          strokeLinecap={pLength > 5 ? "round" : "butt"}
          transform={`rotate(-90 ${size / 2} ${size / 2})`}
        />
        ...
        <Circle
          ...
          strokeDashoffset={-(cLength + pLength)}
          fill="none"
          strokeLinecap={fLength > 5 ? "round" : "butt"}
          transform={`rotate(-90 ${size / 2} ${size / 2})`}
        />
      </Svg>

      {/* التعديل المطلوب هنا: النصوص داخل الدائرة */}
      <View style={{ position: 'absolute', alignItems: 'center' }}>
        {/* السعرات المستهلكة حالياً */}
        ...
        {/* خط فاصل صغير */}
        <View style={{ height: 1, width: 30, backgroundColor: 'rgba(255,255,255,0.2)', marginVertical: 8 }} />

        {/* الهدف والنسبة المئوية */}
        <Text style={{ color: 'white', fontSize: 13, fontWeight: '500' }}>
          Goal: {goalVal}
        </Text>
        <Text style={{ color: currentTotal > goalVal ? '#ef4444' : '#22c55e', fontWeight: 'bold', fontSize: 17, marginTop: 2 }}>
          {Math.round((currentTotal / goalVal) * 100)}%
        </Text>
      </View>
    </View>
  );
...
<View style={{ flexDirection: "row", alignItems: "center" ,marginTop:5}}>
  ...
  <Text style={{ color: "#ffffff", fontSize: 11, marginLeft: 6 }}>
    {Math.round(percent * 100)} %
  </Text>
</View>
  ...
  <View style={{
    ...
    <View style={{flexDirection:"row", alignItems:"center", gap:8}}>
      ...
      <View>
        ...
        <Text style={{fontSize:12,color:"#dfe1e4"}}>
          Protein
        </Text>
      </View>
    </View>
...
{/* DAILY MACROS CARD */}
  ...
  <MacroGoalProgressCircle 
    protein={totals.protein} 
    carbs={totals.carbs} 
    fat={totals.fat} 
    goal={goalCalories} 
    totalConsumed={consumedCalories}
    size={170}
  />

</View>

  {/* شريط التقدم السفلي (اختياري إذا كنت تريد إبقاءه) */}
...
//backgroundColor:"#273d5d",
  ...
  //backgroundColor:"rgba(105, 225, 149, 0.12)",
  ...
  borderColor:"rgba(7, 214, 83, 0.18)",
  shadowColor:"#000",
  shadowOpacity:0.15,
  shadowRadius:20,
